...
double coins(ll i, ll heads, double dp[][3000]){
	// base case
	if(heads == 0){
		return 1;
	}

	if(i == n){ // heads bach gaye but coins khatam
		return 0;
	}
	...
	// recursive case
	double op1 = p[i] * coins(i + 1, heads-1, dp);
	double op2 = (1 - p[i])*coins(i+1, heads, dp);

	return dp[i][heads]=op1 + op2;
}
...
void solve(){
	...
	cout << fixed << setprecision(10) <<coins(0, (n+1)/2 ,dp) << endl;
}
